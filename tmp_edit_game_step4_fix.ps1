...
$start=$text.IndexOf('function normalizeKnockoutStage(')
$end=$text.IndexOf('function updateManagerAfterRound(', $start)
if($start -lt 0 -or $end -lt 0){throw 'normalize/progress markers not found'}
$block=@"
...
function buildGroupStandingsForCompetition(competitionType, competitionName) {
  const fixtures = db.prepare(
    "SELECT f.home_club_id AS homeClubId, f.away_club_id AS awayClubId, f.home_score AS homeScore, f.away_score AS awayScore, f.competition_stage AS competitionStage, home.name AS homeClubName, away.name AS awayClubName, home.strength AS homeStrength, away.strength AS awayStrength FROM fixtures f JOIN clubs home ON home.id = f.home_club_id JOIN clubs away ON away.id = f.away_club_id WHERE f.competition_type = ? AND f.competition_name = ? AND f.status = 'played' AND f.competition_stage LIKE 'Group %'"
