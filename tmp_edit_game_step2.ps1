...
function createGuestPlayersForClub(club, startPlayerId) {
  const insertPlayer = db.prepare(`
    INSERT INTO players (
      ...
      overall, attack, defense, passing, stamina, goalkeeping, wage, value
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
function ensureEuroGuestClubs() {
  ...
  const leagueByCountry = new Map(
    db.prepare("SELECT id, country FROM leagues WHERE tier = 1").all().map((row) => [row.country, row.id])
  );
  ...
  guestBlueprint.forEach((entry) => {
    ...
    if (!leagueId) {
      ...
      nextLeagueId += 1;
      insertLeague.run(leagueId, entry.country, `${entry.country} Premier`, SEASON_LABEL, `${entry.country.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-premier`);
...
$text = [regex]::Replace($text, '(?s)function seedKnockoutCompetitions\(selectedClubId, maxLeagueRound\) \{.*?\}\r?\n\r?\nfunction ensureRuntimeCompetitions\b', $replacement)
